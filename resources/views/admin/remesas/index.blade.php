        ...
        <div class="mb-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex gap-3">
                <a href="{{ route('admin.index') }}"
                    class="flex items-center gap-2 bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold px-5 py-2 rounded-lg shadow transition-all border border-gray-300">
                    ← Regresar
                </a>
                <a href="{{ route('admin.remesas.create') }}"
                    class="flex items-center gap-2 bg-gradient-to-r from-green-500 to-green-700 hover:from-green-600 hover:to-green-800 text-white font-semibold px-5 py-2 rounded-lg shadow transition-all border border-green-600">
                    ➕ Nueva Comisión
                </a>
            </div>
        </div>
                ...
                <table class="min-w-full text-base border-separate border-spacing-y-2">
                    ...
                    <tbody>
                        @forelse($remesas as $r)
                            <tr
                                class="bg-green-50 hover:bg-green-100 transition-colors shadow rounded-xl border-b border-green-100">
                                <td class="p-4 font-semibold text-green-900 border-r border-green-100">{{ $r->nombre }}
                                </td>
                                <td class="p-4 text-green-800 border-r border-green-100">L.
                                    {{ number_format($r->monto_min, 2) }}</td>
                                <td class="p-4 text-green-800 border-r border-green-100">L.
                                    {{ number_format($r->monto_max, 2) }}</td>
                                <td class="p-4 text-green-800 border-r border-green-100">L.
                                    {{ number_format($r->comision, 2) }}</td>
                                <td class="p-4">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('admin.remesas.edit', $r) }}"
                                            class="flex items-center gap-1 bg-amber-600 hover:bg-amber-700 text-white px-4 py-1.5 rounded-lg shadow transition-all border border-amber-700 font-semibold">
                                            ✏️ Editar
                                        </a>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="p-6 text-center text-green-600 bg-green-50 rounded-xl">Sin
                                    registros</td>
                            </tr>
                        @endforelse
                    </tbody>
