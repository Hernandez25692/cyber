    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div class="bg-yellow-100 border border-yellow-200 p-5 rounded-xl shadow-sm flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-yellow-600 mt-1">Precio total</p>
            </div>
            <div class="text-3xl font-bold text-yellow-700">
                L {{ number_format($recargas->sum(fn($r) => $r->paquete->precio ?? 0), 2) }}
            </div>
        </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-yellow-300">
            <thead class="bg-yellow-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-yellow-600 uppercase">Monto</th>
                ...
                @forelse ($recargas as $recarga)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-yellow-800 font-semibold">L
                            {{ number_format($recarga->paquete->precio ?? 0, 2) }}</td>
                        <td class="px-4 py-3 text-gray-500">{{ $recarga->created_at->format('d/m/Y H:i') }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-4 text-center text-sm text-gray-500">No hay recargas.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
