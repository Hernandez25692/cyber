    ...
    <div class="max-w-7xl mx-auto px-4 py-8 bg-gradient-to-br from-gray-100 via-blue-50 to-gray-200 min-h-screen font-sans">
        ...
        <div class="mb-8 flex items-center justify-between border-b-4 border-blue-700 pb-4">
            <div>
                ...
                <p class="text-base text-blue-700 mt-1">Visualiza el historial de remesas procesadas por los cajeros del
                    sistema.</p>
            </div>
            <a href="{{ route('admin.index') }}"
                class="bg-blue-700 text-white px-6 py-2 rounded-lg shadow-lg hover:bg-blue-800 transition flex items-center gap-2 font-semibold">
        ...
        <form method="GET"
            class="mb-6 flex flex-wrap items-end gap-4 bg-white rounded-xl shadow p-4 border border-blue-100">
            <div>
                <label class="block text-xs text-blue-900 font-semibold mb-1" for="cajero">Buscar por Cajero</label>
                <input type="text" name="cajero" id="cajero" value="{{ $filtros['cajero'] ?? '' }}"
                    placeholder="Nombre del cajero..."
                    class="border border-blue-200 rounded px-3 py-2 bg-blue-50 text-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
            </div>
            <div>
                <label class="block text-xs text-blue-900 font-semibold mb-1" for="desde">Fecha Desde</label>
                <input type="date" name="desde" id="desde" value="{{ $filtros['desde'] ?? '' }}"
                    class="border border-blue-200 rounded px-3 py-2 bg-blue-50 text-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
            </div>
            <div>
                <label class="block text-xs text-blue-900 font-semibold mb-1" for="hasta">Fecha Hasta</label>
                <input type="date" name="hasta" id="hasta" value="{{ $filtros['hasta'] ?? '' }}"
                    class="border border-blue-200 rounded px-3 py-2 bg-blue-50 text-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-400" />
            </div>
            <div class="flex gap-2">
                <button type="submit"
                    class="bg-blue-600 text-white px-4 py-2 rounded shadow hover:bg-blue-700 transition font-semibold">Filtrar</button>
                <a href="{{ route('admin.reportes.remesas') }}"
                    class="bg-gray-200 text-blue-700 px-4 py-2 rounded hover:bg-gray-300 transition font-semibold">Limpiar</a>
            </div>
        </form>
        ...
        <div class="grid grid-cols-1">
            <div class="bg-white rounded-2xl shadow-xl border border-blue-200 p-0 overflow-hidden">
                ...
                <div class="overflow-x-auto">
                        ...
                        <tbody>
                            @forelse($remesas as $r)
                                <tr class="border-b border-blue-100 hover:bg-blue-50 transition-colors">
                                    ...
                                    <td class="p-3 whitespace-nowrap text-blue-700">L. {{ number_format($r->comision, 2) }}
                                    </td>
                                    <td class="p-3 whitespace-nowrap">{{ $r->referencia ?? '---' }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center text-blue-400 py-6 font-semibold">No hay registros
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                    ...
                    <div class="px-6 py-4">
                        {{ $remesas->appends(request()->query())->links() }}
                    </div>
