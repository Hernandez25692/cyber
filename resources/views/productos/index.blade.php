    ...
    <div class="max-w-7xl mx-auto px-4 py-6 bg-gray-100 min-h-screen">
        ...
        <div class="mb-8 flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Administra y visualiza todos los productos registrados en el sistema.
                </p>
            </div>
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-3">
            <div class="flex gap-3">
                ...
                {{-- 🔹 NUEVO: Exportar a Excel (usa los filtros actuales) --}}
                <a href="{{ route('admin.productos.export', request()->query()) }}"
                    class="bg-green-600 text-white px-5 py-2 rounded shadow hover:bg-green-700 flex items-center gap-2 font-semibold">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M4 4h16v16H4zM8 8h8M8 12h5" />
                    </svg>
                    Exportar Excel
                </a>
            </div>
        ...
        <!-- Filtros -->
        <form method="GET"
            class="bg-white p-4 rounded-lg shadow mb-6 grid grid-cols-1 md:grid-cols-5 gap-4 text-sm border border-gray-200">
            ...
            <select name="stock"
                ...
                <option value="disponible" {{ request('stock') == 'disponible' ? 'selected' : '' }}>Disponible (&gt; 5)
                </option>
            </select>
                ...
                <table class="min-w-full text-sm border-separate border-spacing-y-2">
                    ...
                    <tbody>
                        @forelse ($productos as $producto)
                            <tr
                                class="bg-white hover:bg-blue-50 border border-gray-200 rounded-lg shadow-sm transition-all">
                                ...
                                <td class="p-3 text-right text-gray-700">L.
                                    {{ number_format($producto->precio_compra, 2) }}</td>
                                <td class="p-3 text-right text-green-700 font-semibold">L.
                                    {{ number_format($producto->precio_venta, 2) }}</td>
                                <td class="p-3 text-center">
                                    @if ($producto->stock == 0)
                                        <span class="bg-red-100 text-red-700 px-2 py-1 rounded text-xs font-bold">Sin
                                            stock</span>
                                    @elseif($producto->stock <= 10)
                                        <span
                                            class="bg-yellow-100 text-yellow-800 px-2 py-1 rounded text-xs font-bold">Bajo
                                            ({{ $producto->stock }})</span>
                                    @else
                                        <span
                                            class="bg-green-100 text-green-800 px-2 py-1 rounded text-xs font-bold">{{ $producto->stock }}</span>
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="p-6 text-center text-gray-400">No hay productos registrados.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
